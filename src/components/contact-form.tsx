...
export function ContactForm() {
  ...
  return <form className="contact-form" onSubmit={submit} aria-describedby="contact-form-note">
    <div className="contact-form-heading"><div className="contact-form-heading-icon" aria-hidden="true">↗</div><div><h2>Send Rohan a message</h2><p id="contact-form-note">Share the context once. Your enquiry goes directly to Rohan.</p></div></div>
    <div className="contact-form-fields">
      ...
      <fieldset className="contact-service-fieldset contact-form-wide"><legend>What can GrowthLabs help with? *</legend><div className="contact-service-grid">{[...services, { title: "Not sure yet", slug: "not-sure" }].map((service, index) => <label key={service.slug}><input name="service" type="radio" required value={service.title} /><span className="contact-service-choice"><span className="contact-service-icon" aria-hidden="true">{serviceIcons[index]}</span><span>{service.title}</span></span></label>)}</div></fieldset>
