...
export function CommercialLandingPage({ page }: { page: CommercialLanding }) {
  ...
  const primary = page.primaryCta ?? { label: "Talk to Rohan", href: "/contact" };
  ...
  const finalSecondary = page.finalSecondaryCta ?? { label: "WhatsApp Rohan", href: siteConfig.whatsapp, external: true };

  return <>
    <PageIntro breadcrumbs={[{ label: "Services", href: "/services" }, { label: currentLabel }]} eyebrow={page.eyebrow} title={page.title}><p>{page.intro}</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link className="button button-primary" href={primary.href}>{primary.label} <Arrow /></Link><Link className="button" href={secondary.href}>{secondary.label} <Arrow /></Link></div>{page.microcopy && <p className="mt-4 font-mono text-[.7rem] uppercase leading-5 tracking-[.04em]">{page.microcopy}</p>}</PageIntro>
