...
export function ContactCta() {
  return <section className="section signal-grid"><div className="shell grid gap-10 md:grid-cols-[1fr_3fr]"><p className="eyebrow">Founder-led next step</p><div><h2 className="section-title">Bring the growth problem. Find the clearest next move.</h2><p className="mt-6 max-w-2xl text-lg leading-8 text-black/65">{siteConfig.cta.microcopy}</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href={siteConfig.cta.primary.href} className="button button-primary" data-cta-intent="growth-review" data-cta-location="shared-final" data-cta-channel="contact">{siteConfig.cta.primary.headerLabel} <Arrow /></Link><a className="button" href={siteConfig.cta.whatsapp.href} target="_blank" rel="noopener noreferrer" data-cta-intent="growth-review" data-cta-location="shared-final" data-cta-channel="whatsapp">{siteConfig.cta.whatsapp.label} <Arrow direction="up-right" /></a></div><p className="mt-5 font-mono text-[.72rem] uppercase leading-5 tracking-[.04em] text-black/65">{siteConfig.cta.reassurance}</p></div></div></section>;
}
