...
export default function sitemap(): MetadataRoute.Sitemap {
  const routes = ["", "/services", "/digital-marketing-agency-in-nepal", "/seo-expert-in-nepal", "/seo-company-in-nepal", ...commercialLandings.map((page) => `/${page.slug}`), "/insights", ...insights.map((article) => `/insights/${article.slug}`), "/about", "/contact", "/privacy"];
