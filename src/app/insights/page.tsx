...
export default function InsightsPage() {
  ...
  return <>
    ...
    <section className="section scroll-mt-24" id="articles"><div className="shell"><SectionHeading eyebrow="Articles / Start here" title="Understand SEO before buying SEO." copy="The beginner pillar separates crawling, indexing, ranking, traffic and business outcomes, then connects the concepts to a practical evaluation process." /><Link className="insight-feature mt-12 grid overflow-hidden bg-[var(--ink)] text-[var(--paper)] transition-transform hover:-translate-y-1 md:ml-[18%] lg:grid-cols-[1.05fr_1.35fr]" href={`/insights/${featured.slug}`}><div className="insight-feature-media relative border-b border-white/15 lg:border-b-0 lg:border-r"><Image className="h-full w-full object-contain" src={agencyAssets.seoServices.src} alt={agencyAssets.seoServices.alt} width={agencyAssets.seoServices.width} height={agencyAssets.seoServices.height} sizes="(max-width: 1024px) 100vw, 42vw" /><span className="absolute left-4 top-4 bg-[var(--paper)] px-3 py-2 font-mono text-[.68rem] font-bold uppercase text-[var(--ink)]">01 / {featured.category}</span></div><div className="flex min-h-[25rem] flex-col justify-between p-7 md:p-10 lg:p-12"><div><p className="font-mono text-xs font-bold uppercase text-[var(--sun)]">{featured.readingTime}</p><h2 className="mt-6 max-w-[18ch] font-serif text-[clamp(2.35rem,4.3vw,4.75rem)] leading-[.96] tracking-[-.055em]">{featured.title}</h2><p className="mt-6 max-w-2xl leading-7 text-white/65">{featured.description}</p></div><span className="mt-10 flex items-center gap-3 font-bold text-[var(--sun)]">Read article <Arrow /></span></div></Link></div></section>
    ...
    {categoryOrder.map((category) => {
      ...
      return <section className="section" id={category.toLowerCase().replaceAll(" ", "-")} key={category}><div className="shell"><div className="grid gap-7 md:grid-cols-[1fr_3fr]"><p className="eyebrow">{category} articles</p><div className="insight-card-grid grid gap-5 sm:grid-cols-2">{categoryArticles.map((article, index) => { const visual = agencyAssets[insightVisuals[article.category] ?? "growthSystem"]; return <Link className="insight-card group flex min-h-72 flex-col overflow-hidden transition-colors" href={`/insights/${article.slug}`} key={article.slug}><div className="insight-card-media overflow-hidden border-b border-[var(--line)] bg-[var(--paper-deep)]"><Image className="h-full w-full object-contain transition-transform duration-300 group-hover:scale-[1.02]" src={visual.src} alt="" width={visual.width} height={visual.height} sizes="(max-width: 640px) 100vw, 40vw" /></div><div className="flex flex-1 flex-col p-6"><p className="font-mono text-[.65rem] font-bold uppercase tracking-[.045em] text-[var(--accent)]">{String(index + 1).padStart(2, "0")} · {article.readingTime}</p><h2 className="mt-6 max-w-[18ch] font-serif text-3xl leading-[1] tracking-[-.05em]">{article.shortTitle}</h2><p className="mt-4 text-sm leading-6 text-[var(--muted)]">{article.description}</p><span className="mt-auto flex items-center gap-2 pt-8 text-sm font-bold">Read article <Arrow /></span></div></Link>; })}</div></div></div></section>;
    })}
