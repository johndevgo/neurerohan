...
export default function InsightsPage() {
  ...
  return <>
    <PageIntro breadcrumbs={[{ label: "Insights" }]} eyebrow="Guides · Questions · Glossary · Case studies" title="Useful answers for better growth decisions." visual={agencyAssets.growthSystem}><p>Original guides, questions, definitions, case-study evidence, checklists and tools across SEO, local search, social media, paid acquisition, websites and measurement. Each resource answers the question first and leads to a practical next step.</p><div className="mt-7 flex flex-wrap gap-2">{[["Questions", "/insights/questions"], ["Glossary", "/insights/glossary"], ["Case studies", "/insights/case-studies"], ["Resources", "/resources"], ["Tools", "/tools"]].map(([label, href]) => <Link className="border border-[var(--line-strong)] px-3 py-2 text-xs font-bold hover:bg-[var(--sun)]" href={href} key={href}>{label}</Link>)}</div></PageIntro>
