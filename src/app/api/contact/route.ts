...
export async function POST(request: NextRequest) {
  ...
  const recipients = [...new Set([recipient, backupRecipient].filter(Boolean))];
  const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json", "Idempotency-Key": randomUUID(), "User-Agent": "GrowthLabs-Website/1.0" }, body: JSON.stringify({ from: sender, to: recipients, reply_to: email, subject, text, html, tags: [{ name: "source", value: "contact-form" }] }) });
  if (!response.ok) {
  ...
  console.info("Contact delivery accepted", providerResult.id);
  return NextResponse.json({ message: "Your enquiry was accepted for delivery.", reference: providerResult.id });
}
