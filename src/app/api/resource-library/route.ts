...
export async function POST(request: NextRequest) {
  ...
  const sender = process.env.CONTACT_FROM_EMAIL?.trim() || "GrowthLabs Library <resources@send.neurerohan.com.np>";
  ...
  const delivery = await sendEmail(apiKey, { from: sender, to: [email], reply_to: recipient, subject: "Your complete GrowthLabs marketing tools & resources library", html, text });
  if (!delivery.ok) {
  ...
  const notification = await sendEmail(apiKey, { from: sender, to: [recipient], reply_to: email, subject: `GrowthLabs library request — ${email}`, html: notice, text: `Resource library request\nEmail: ${email}\nName: ${name || "Not supplied"}\nSource: ${sourcePath}` });
  if (!notification.ok) console.error("Resource library notification failed", notification.status, await notification.text());

  return NextResponse.json({ message: "The complete library was sent." });
}
